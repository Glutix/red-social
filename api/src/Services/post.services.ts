...
export const createPost = async (postInput: PostProps): Promise<Post> => {
  ...
  try {
    ...
    if (!content) {
      ...
      throw new Error("No puedes crear una publicación si no iniciaste sesión.")
    }
...
export const getPostsByUserId = async (userId: number) => {
  ...
  try {
    ...
    if (!userPosts || userPosts.length === 0) {
      throw new Error(`No existen publicaciones para el usuario con el id ${userId}`);
    }
...
export const updatePost = async (postId: number, postInput: Partial<PostProps>): Promise<Post> => {
  try {
