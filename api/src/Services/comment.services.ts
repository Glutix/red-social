...
export const createComment = async (commentInput: CommentProps): Promise<Comment> => {
...
export const updateComment = async (commentID: number, commentInput: Partial<CommentProps>): Promise<Comment> => {
  try {
    const comment = await Comment.findByPk(commentID);
